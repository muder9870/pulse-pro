...
- [ ] 1. Add `RelevanceKeyword` model and migration
  - [ ] 1.1 Add `RelevanceKeyword` SQLAlchemy model to `backend/db/models.py`
  ...
  - [ ] 1.2 Create Alembic migration for `relevance_keywords` table
...
- [ ] 2. Create `KeywordRepository`
  - [ ] 2.1 Create `backend/db/repositories/keyword_repository.py`
  ...
  - [ ] 2.2 Wire `seed_defaults()` into app startup in `backend/main.py`
...
- [ ] 3. Create Keywords API blueprint
  - [ ] 3.1 Create `backend/api/routes/keywords.py` with `keywords_bp`
  ...
  - [ ] 3.2 Register `keywords_bp` in `backend/main.py`
    - Import `keywords_bp` from `backend.api.routes.keywords` and call `app.register_blueprint(keywords_bp)`
    - _Requirements: 5.1_

- [ ] 4. Update `GenerateRequest` schema and generate route
  - [ ] 4.1 Update `GenerateRequest` in `backend/api/schemas.py`
  ...
  - [ ] 4.2 Update `generate_content` route in `backend/api/routes/` to pass `platforms` to `ContentGenerator`
...
- [ ] 5. Strip generate and schedule stages from orchestrator
  - [ ] 5.1 Remove `generate` and `schedule` stages from `run_full_pipeline` in
  ...
  - [ ]* 5.2 Write property test: pipeline produces no generated content (Property 1)
  ...
  - [ ]* 5.3 Write property test: pipeline result shape (Property 2)
...
- [ ] 6. Change `ANALYSIS_LIMIT` default to 0
  - [ ] 6.1 Update `ANALYSIS_LIMIT` default in `backend/config.py` from `3` (or `50`) to `0`
    - _Requirements: 2.1, 2.2_

  - [ ] 6.2 Update `AnalysisAgent.run()` in `backend/agents/analysis_agent.py` to treat `0` as "no cap"
  ...
  - [ ]* 6.3 Write property test: analysis limit is respected (Property 3)
...
- [ ] 7. Integrate keyword scoring into scorer
  - [ ] 7.1 Add `calculate_keyword_boost()` to `backend/processors/scorer.py`
  ...
  - [ ] 7.2 Update `calculate_relevance_score()` to apply keyword boost, capped at 100
    - Accept optional `keywords: list[str] = None` parameter
    - `return min(base + self.calculate_keyword_boost(..., keywords or []), 100)`
    - _Requirements: 3.5, 3.6_

  - [ ] 7.3 Update `score_all_articles()` to load keywords once before the scoring loop
  ...
  - [ ]* 7.4 Write property test: keyword score is a pure function of match count (Property 4)
  ...
  - [ ]* 7.5 Write property test: relevance score capped at 100 (Property 5)
  ...
  - [ ]* 7.6 Write property test: keyword matching is case-insensitive substring (Property 6)
...
- [ ] 9. Add Generate button and PlatformSelector to `StoryCard.jsx`
  - [ ] 9.1 Add state and Generate button to `frontend/src/components/StoryCard.jsx`
  ...
  - [ ] 9.2 Implement `PlatformSelectorDropdown` inline in `StoryCard.jsx` and wire confirm action
  ...
  - [ ] 9.3 Add bulk selection checkboxes to StoryCards and bulk action toolbar to Dashboard
...
- [ ] 10. Create `KeywordsManager.jsx`
  - [ ] 10.1 Create `frontend/src/components/KeywordsManager.jsx`
...
- [ ] 11. Add Keywords tab to `SettingsView.jsx`
  - [ ] 11.1 Import `KeywordsManager` and add Keywords tab entry in `frontend/src/components/SettingsView.jsx`
...
- [ ] 12. Update pipeline stage labels in `AdvancedTools.jsx`
  - [ ] 12.1 Replace any "Generating" / "Scheduling" stage labels in
    `frontend/src/components/AdvancedTools.jsx` with the new set: `Fetching` → `Analyzing` →
    `Scoring` → `Done`
...
- [ ] 13. Write property-based tests for keyword scoring (Hypothesis)
  - [ ]* 13.1 Write property test: keyword uniqueness (Property 7)
  ...
  - [ ]* 13.2 Write property test: keyword API round-trip (Property 8)
  ...
  - [ ]* 13.3 Write property test: bulk insert skips duplicates (Property 9)
  ...
  - [ ]* 13.4 Write property test: platform-selective generation (Property 10)
  ...
  - [ ]* 13.5 Write property test: existing generated content is preserved (Property 11)
...
- [ ] 14. Write frontend tests for StoryCard platform selector (Vitest + React Testing Library)
  - [ ]* 14.1 Create `frontend/src/components/StoryCard.test.jsx`
  ...
  - [ ]* 14.2 Create `frontend/src/components/KeywordsManager.test.jsx`
...
- [ ] 15. Delete dead code files
  - [ ] 15.1 Verify zero active imports, then delete any auto-generation logic files removed from
    the orchestrator
...
- [ ] 16. Update technical documentation
  - [ ] 16.1 Update `README.md`, `TECHNICAL_ARCHITECTURE.md` (or equivalent), and `RUNBOOK.md`
    to reflect the new two-phase architecture: pipeline stops after scoring; content generation
    is user-triggered via `POST /api/generate`
    - Document the new `GET/POST/DELETE /api/keywords` endpoints
    - Document `ANALYSIS_LIMIT` default change (0 = all)

- [ ] 17. Final verification
  - [ ] 17.1 Rebuild Docker and run a clean state reset
    - `POST /api/system/reset-data` with `X-Reset-Confirm: yes`
    - Rebuild Docker image after all backend changes
  - [ ] 17.2 Trigger a fresh pipeline run and verify pipeline isolation
    - Run `POST /api/pipeline/run` and assert `generated_content` table has 0 new rows
    - Assert `scheduled_posts` table has 0 new rows
    - _Requirements: 1.4_
  - [ ] 17.3 Verify user-triggered generation produces exactly the requested rows
