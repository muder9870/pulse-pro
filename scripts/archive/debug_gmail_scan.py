...
def debug_scan():
    ...
    for m_id in reversed(last_ids):
        ...
        status_str = "✅ AI NEWSLETTER" if (is_ai and is_newsletter) else "❌ REJECTED"
        if not is_ai: status_str += " (Not AI)"
        if not is_newsletter: status_str += " (Not Newsletter)"
        
        print(f"\n- From: {sender_email}")
        print(f"  Subject: {subject}")
        print(f"  Status: {status_str} | Confidence: {confidence:.2f}")
