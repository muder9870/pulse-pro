...
- Add exponential backoff with randomized jitter for provider retries (cap max delay).
- Fix `fallback` boolean so it is `true` when a fallback provider was used (analytics integrity).
- Implement a global provider cooldown when circuit-breaker opens (30–60s open window) and route calls to fallback.
- Add minimal metrics instrumentation: provider success rate, p95 latency, retry count, fallback percentage, queue wait time.
- Ensure circuit-breaker state is shared across workers (Redis or DB) instead of per-process.

## Short-term / Medium
- Implement a Redis-backed global rate limiter (token bucket or sliding window) for Cerebras calls.
- Add distributed semaphore or inflight request limiter to bound concurrency to provider budget.
- Separate queues by priority (light / heavy / retry) or use routing to reduce thundering-herd surface.
- Honor `Retry-After` / rate-limit response headers from Cerebras when present.
- Add request-level jitter to avoid synchronized retry storms across workers.
...
## Future / Nice-to-have
- Dynamic provider load balancing and cost-aware routing between Cerebras / Mistral.
