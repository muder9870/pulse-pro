...
class Deduplicator:
        ...
        with get_session() as session:
            ...
            for i in range(n):
                ...
                result = session.execute(
                    ...
                    .where(RawArticle.id == article_i.id, RawArticle.state == 'cleaned')
                ...
                for j in range(i + 1, n):
                    ...
                    if article_i.url and article_j.url and article_i.url == article_j.url:
                        self._mark_pair(session, article_i, article_j)
                        marked += 1
                        continue

                    # Fuzzy title match
                    if self.are_duplicates(article_i.title, article_j.title):
                        self._mark_pair(session, article_i, article_j)
                        marked += 1

            # After scanning all pairs, mark any remaining 'deduplicating' as 'deduped'
            session.execute(
                sql_update(RawArticle)
                .where(RawArticle.state == 'deduplicating')
                .values(state='deduped')
            )
            session.commit()

        return marked

    def are_duplicates(self, title1: str, title2: str) -> bool:
        """Check if two titles are duplicates using Jaccard similarity on cleaned word sets."""
        s1_words = self._clean_to_words(title1)
        s2_words = self._clean_to_words(title2)
    ...
    def _mark_pair(self, session, article_a: RawArticle, article_b: RawArticle) -> None:
